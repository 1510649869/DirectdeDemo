shared uniform extern float4x4  gProjection:PROJECTION;
shared uniform extern float     gTime;
shared uniform extern int       gTotalLights;
struct Light
{
	float enabled;
	float lightType;
	...
	float3 position;
	float3 direction;
	float4 spotData;    //四个分量分别保存range falloff theta,phi
};
//光源数组
